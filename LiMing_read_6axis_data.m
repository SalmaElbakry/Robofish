function [TL,TF]=LiMing_read_6axis_data(Flow_Speed, SAMPLE_FREQUENCY, frequency, Nor_Amp, Phase_lag, COLLECT_TIME, DX, DY, i)
    % Constants
    ...
    SAVE_DIR = 'D:\'; %% change here for save dir
    ...
    try
        ...
        for idx = 1:length(COM_PORTS)
            ...
            if extendet ~= 0
                error(['Activation error: ', num2str(extendet), ' for COM', num2str(com)]);
            else
                disp(['COM', num2str(com), ' activated successfully.']);
            end

            % Set the sampling rate
            freqSetResult = calllib('MEGSV86x64', 'GSV86setFrequency', com, SAMPLE_FREQUENCY);
            if freqSetResult ~= 0
                error(['Frequency set error: ', num2str(freqSetResult), ' for COM', num2str(com)]);
            else
                disp(['Frequency set to ', num2str(SAMPLE_FREQUENCY), ' Hz successfully for COM', num2str(com)]);
            end
            ...
            [~, ~] = calllib('MEGSV86x64', 'GSV86readMultiple', com, 0, out, count, valsread, ErrFlags);
            num_vals_read = get(valsread, 'Value');
            data = get(out, 'Value');
            data = data(1:num_vals_read);

            % Process data
            if mod(num_vals_read, NUM_MAPPED_OBJECTS) == 0
                channel_data = reshape(data, NUM_MAPPED_OBJECTS, [])';
                voltage_signals = channel_data(:, 1:6);
                forces_torques = voltage_signals * calibration_matrix';
                file_name = sprintf('Flow=%.2f_Fre=%d_Amp=%.2f_Phase=%.2f_DX=%.2f_DY=%.2f_%s_i=%d.csv', ...
                    Flow_Speed, frequency, Nor_Amp, Phase_lag, DX, DY, role, i);
                file_path = fullfile(SAVE_DIR, file_name);
                writematrix(forces_torques, file_path);
                disp(['Force/Torque data saved for COM', num2str(com), ' to ', file_path]);
            else
                disp(['Data length is not a multiple of NUM_MAPPED_OBJECTS for COM', num2str(com)]);
            end
