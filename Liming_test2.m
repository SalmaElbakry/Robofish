...
try
    ...
    for idx = 1:length(COM_PORTS)
        ...
        % Check for errors
        if error ~= 1
            error(['GSV86readMultiple error: ', num2str(error), ' for ', port_name]);
        end
        ...
        if mod(num_vals_read, NUM_MAPPED_OBJECTS) == 0
            ...
            % Save data
            save(['forces_torques_', port_name, '.mat'], 'forces_torques');
            writematrix(forces_torques, ['forces_torques_', port_name, '.csv']);
            disp(['Force/Torque data saved for ', port_name, '.']);
        else
