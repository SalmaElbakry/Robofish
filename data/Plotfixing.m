...
rigidData = load('RIGID_flow_0_angle_10_fre_1_dist_7.mat', 'TorFor_F', 'TorFor_L');
...
for i = 1:6 % For each axis (Fx, Fy, Fz, Tx, Ty, Tz)
    ...
    thrust_F_soft = zeros(9, 1);
    thrust_L_soft = zeros(9, 1);
    thrust_F_rigid = zeros(9, 1);
    thrust_L_rigid = zeros(9, 1);

    for k = 1:9
        % Max over time (row-wise), not trials
        thrust_F_soft(k) = max(TorFor_F_soft{k, j}(:, i));
        thrust_L_soft(k) = max(TorFor_L_soft{k, j}(:, i));
        
        thrust_F_rigid(k) = max(TorFor_F_rigid{k, j}(:, i));
        thrust_L_rigid(k) = max(TorFor_L_rigid{k, j}(:, i));
    end

    % No std ? only one run per condition
    subplot(2, 3, i)
    plot(1:9, thrust_F_soft, '-ob', 'DisplayName', 'soft Follower');
    hold on
    plot(1:9, thrust_L_soft, '-og', 'DisplayName', 'soft Leader');
    plot(1:9, thrust_F_rigid, '-or', 'DisplayName', 'rigid Follower');
    plot(1:9, thrust_L_rigid, '-ok', 'DisplayName', 'rigid Leader');
...
%     hold on
% %     errorbar(1:9, mean_L_soft, ustd_L_soft, '-og', 'DisplayName', 'soft Leader');
